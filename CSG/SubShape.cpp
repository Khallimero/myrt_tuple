...
SubShape::SubShape(SmartPointer<const Shape> s1,SmartPointer<const Shape> s2,bool merge)
    :CSGShape(s1,s2,merge) {}

SubShape::~SubShape() {}

bool SubShape::isInside(const Point& p,double e)const
{
    return (shapes[CSGShape::first].s->isInside(p,e))&&(!shapes[CSGShape::second].s->isInside(p,-e));
}

Hit SubShape::__getHit(const Ray& r)const
{
    Ray rr=r;
    ...
    do
    {
        ...
        if(h2.isNull()&&shapes[CSGShape::second].s->isInside(rr.getPoint()))return Hit::null;

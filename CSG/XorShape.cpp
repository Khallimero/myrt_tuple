...
XorShape::XorShape(SmartPointer<const Shape> s1,SmartPointer<const Shape> s2,bool merge)
    :CSGShape(s1,s2,merge) {}

XorShape::~XorShape() {}

bool XorShape::isInside(const Point& p,double e)const
{
    return (shapes[CSGShape::first].s->isInside(p,e))^(shapes[CSGShape::second].s->isInside(p,e));
}

Hit XorShape::__getHit(const Ray& r)const
{
    Ray rr=r;
